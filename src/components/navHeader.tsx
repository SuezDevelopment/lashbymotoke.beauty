...
export const SessionBookingModal = ({
  ...
  sessionBookingState?: SessionBookingFormData;
  onClose?: () => void;
}) => {
  const [step, setStep] = useState(1);
  const [errorMsg, setErrorMsg] = useState<string | undefined>(undefined);
  const [isSubmitSucess, setIsSubmitSucess] = useState<boolean | undefined>(
    undefined
  );
          ...
          {step === 1 && (
            <div className="flex flex-col md:flex-row gap-6">
              ...
              <div className="w-full md:w-full flex flex-col gap-6 p-8 md:px-8">
                ...
                <button
                  onClick={() => handleNext()}
                  className="w-full py-3 bg-[#a68ea5] font-semibold text-white rounded-lg hover:bg-[#957994] transition-colors"
                >
                  Continue
                </button>
              </div>
            ...
            <div className="flex flex-col md:flex-row gap-6">
              ...
              <div className="w-full md:w-full flex flex-col gap-6 p-8 md:px-8">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <button
                    onClick={() => handleBack()}
                    className="w-full py-3 border border-[#a68ea5] font-semibold text-black rounded-lg transition-colors"
                  >
                    Back
                  </button>
                </div>
              ...
              <div className="w-full md:w-full flex flex-col gap-6 p-8 md:px-12 overflow-y-auto">
                <div className="border-b border-black/30 pb-4">
                  ...
                  <p className="text-black/50 text-sm mt-3">
                    Confirm that the information entered are correct, and the
                    price for the session selected.
                  </p>
                </div>
                  ...
                  <div className="border-b border-black/20 pb-2">
                    ...
                    <div>
                      {formData.serviceType == "studio"
                        ...
                        ? "Island (Victoria island, Ikoyi, Banana island)"
                        : formData.scheduleLocation == "island-2"
                        ? "Island (Lekki, Ikate, Chevron)"
                        : "Island (Ajah, Lekki garden)"}
                    </div>
                  ...
                  <div className="text-center space-y-4">
                    <h3
                      className={`text-2xl font-bold ${
                        isSubmitSucess ? "text-[#a68ea5]" : "text-red-500"
                      }`}
                    >
                      {isSubmitSucess
                        ? `Booking Successful!`
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 w-full backdrop-blur-sm bg-white/30 shadow-lg px-4 py-4 sm:px-8 lg:px-20 flex items-center justify-between z-20">
        ...
        {isMenuOpen && (
          <div
            className={`lg:hidden absolute top-full z-20 left-0 w-full h-[100vh] bg-white shadow-md transform transition-transform duration-500  ${
            ...
            <ul className="py-10 px-6 flex flex-col items-start justify-between h-[50vh]">
              ...
              <li className="w-full">
                <button
                  ...
                  className="px-6 py-2 rounded-2xl border border-black/25 text-black text-xl font-normal leading-relaxed tracking-tight w-full"
                >
                  Book a session
                </button>
              </li>
      ...
      {isModalOpen && <SessionBookingModal setIsModalOpen={setIsModalOpen} />}
    </>
  );
};
...
const CloseButton = ({
  ...
  <button onClick={onClick} className=" hover:text-gray-700">
    <svg
      className={`w-${w} h-${h}`}
      fill="none"
      stroke="#000000"
